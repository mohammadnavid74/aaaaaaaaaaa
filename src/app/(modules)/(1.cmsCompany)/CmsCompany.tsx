...
interface MapProps {
  ...
  cmsCompaniesData: CMSCompanySummaryDto[];
}

const CmsCompany = ({ updateSharedValue, cmsCompaniesData }: MapProps) => {
  ...
  useEffect(() => {
    let filtered = cmsCompaniesData.filter((item) => item.status !== 100);
  ...
  }, [cmsCompaniesData, selectedStatuses]);
  ...
  const markerMessages = filteredList.map((item) => item.city);
